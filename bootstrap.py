...
def _detect_command(argv: list[str]) -> str:
    """
    Determine subcommand (bootstrap/teardown). If none is provided, default to bootstrap.
    This parser intentionally ignores module-specific flags.
    """
    pre = argparse.ArgumentParser(add_help=False)
    pre.add_argument("--env-file")
    pre.add_argument("--env-name")
    pre.add_argument("--kube-context")
    pre.add_argument("--non-interactive", action="store_true")
    pre.add_argument("--yes", action="store_true")
    pre.add_argument("command", nargs="?", choices=["bootstrap", "teardown"])
    pre_args, _ = pre.parse_known_args(argv)
    return pre_args.command or "bootstrap"

def _normalize_tsig_alg_for_cert_manager(v: str) -> str:
    # cert-manager expects e.g. HMACSHA256
    x = v.strip()
    if not x:
        return ""
    # common bind-style values seen in your env: hmac-sha256, hmac-sha512, etc.
    m = re.fullmatch(r"(?i)hmac[-_]?sha(\d+)", x)
    if m:
        return f"HMACSHA{m.group(1)}"
    return x  # if already HMACSHA256 or something else, leave it

def _write_platform_env_file(path: Path, values: Dict[str, str]) -> None:
    lines = [
        "# Generated by bootstrap.py",
        "# GitOps platform values (non-secret). Safe to commit.",
        "",
    ]
    for k in sorted(values.keys()):
        lines.append(f"{k}={_quote_env_value(values[k])}")
...
def _derive_platform_env(ctx: Context) -> Dict[str, str]:
    def get(k: str) -> str:
        return (ctx.env_values.get(k) or ctx.existing_env.get(k) or "").strip()

    # cert-manager RFC2136 settings come from CM_* keys in your bootstrap env
    host = get("CM_RFC2136_HOST")
    port = get("CM_RFC2136_PORT") or "5335"
    keyname = get("CM_RFC2136_TSIG_KEYNAME")
    alg_raw = get("CM_RFC2136_TSIG_ALG")

    out: Dict[str, str] = {}

    # step-ca ACME values (if you’ve added them to bootstrap env; otherwise these remain empty)
    step_server = get("STEP_ACME_SERVER")
    acme_email = get("ACME_EMAIL")

    if step_server:
        out["STEP_ACME_SERVER"] = step_server
    if acme_email:
        out["ACME_EMAIL"] = acme_email

    if host:
        out["RFC2136_NAMESERVER"] = f"{host}:{port}" if ":" not in host else host
    if keyname:
        out["RFC2136_TSIG_KEYNAME"] = keyname
    if alg_raw:
        out["RFC2136_TSIG_ALGORITHM"] = _normalize_tsig_alg_for_cert_manager(alg_raw)

    canary = get("CANARY_DNSNAME")
    if canary:
        out["CANARY_DNSNAME"] = canary
...
def main() -> int:
    global_p = argparse.ArgumentParser(
        prog="bootstrap.py",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Bootstrap or teardown Argo CD and core components.",
        add_help=False,
    )
    global_p.add_argument("--env-file", help="Path to write/read env file")
    global_p.add_argument("--env-name", help="Name of environment (resolves to bootstrap/env/<name>.env)")
    global_p.add_argument("--non-interactive", action="store_true", help="Do not prompt")
    global_p.add_argument("--yes", action="store_true", help="Accept defaults without prompting")
    global_p.add_argument("--kube-context", help="Optional kubectl context")
    global_p.add_argument("--write-platform-env", action="store_true",
                   help="Also write gitops/env/<ENV>/platform.env (non-secret values)")
    global_p.add_argument("--platform-env-path", help="Override platform.env output path")

    p = argparse.ArgumentParser(parents=[global_p])

    subparsers = p.add_subparsers(dest="command", help="Command to run")

    # Bootstrap subcommand
    bootstrap_p = subparsers.add_parser("bootstrap", help="Run bootstrap (default)", parents=[global_p])
    
    # Teardown subcommand
    teardown_p = subparsers.add_parser("teardown", help="Run teardown", parents=[global_p])
    ...
    if cmd == "bootstrap":
        args = p.parse_args(["bootstrap", *argv]) if "teardown" not in argv else p.parse_args(["teardown", *argv])
    else:
    ...
    if args.command == "teardown":
        ...
        # Write GitOps platform.env (non-secret)
        env_name = (ctx.env_values.get("ENV") or ctx.existing_env.get("ENV") or "kubeadm").strip()
        default_platform_env = ctx.repo_root / "gitops" / "env" / env_name / "platform.env"

        platform_env_path = Path(args.platform_env_path).expanduser() if getattr(args, "platform_env_path", None) else default_platform_env
        if not platform_env_path.is_absolute():
            platform_env_path = (ctx.repo_root / platform_env_path).resolve()

        platform_values = _derive_platform_env(ctx)
        if platform_values:
            _write_platform_env_file(platform_env_path, platform_values)
            print(f"Platform env:    {platform_env_path}")
        else:
            print("Platform env:    (skipped; no platform values found)")
